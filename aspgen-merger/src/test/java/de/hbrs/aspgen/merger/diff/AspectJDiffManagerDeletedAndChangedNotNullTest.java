...
public class AspectJDiffManagerDeletedAndChangedNotNullTest {
    ...
    public void changedBlock() throws IOException {
        final String fileContent = FileUtils.readFileToString(new File("src/test/resources/ajdiff/changedAndDeleted/notnull/Person_NotNull_Deleted_Block.aj"));
        final String generatedFromJavaFile = FileUtils.readFileToString(new File("src/test/resources/ajdiff/changedAndDeleted/notnull/Person_NotNull_Original.aj"));
        final String generatedFromJavaFile2 = FileUtils.readFileToString(new File("src/test/resources/ajdiff/changedAndDeleted/notnull/Person_NotNull.aj"));
        ...
        assertEquals("NotNull2", result.getAspectJParameterDiffs().get(0).getAnnotationData().getModified().get(0));

        assertEquals("2", result.getAspectJParameterDiffs().get(0).getAnnotationData().getId());
    }
}
