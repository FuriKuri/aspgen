...
public class AspectJDiffManagerDeletedAndChangedLoggingTest {
    ...
    @Test
    public void changedAndBlockBlock() throws IOException {
        final String fileContent = FileUtils.readFileToString(new File("src/test/resources/ajdiff/changedAndDeleted/logging/Person_Logging_Deleted_Blocks.aj"));
        final String generatedFromJavaFile = FileUtils.readFileToString(new File("src/test/resources/ajdiff/changedAndDeleted/logging/Person_Logging_Original.aj"));
        final String generatedFromJavaFile2 = FileUtils.readFileToString(new File("src/test/resources/ajdiff/changedAndDeleted/logging/Person_Logging.aj"));
        ...
        assertEquals("method(Integer, Integer, String)", result.getAspectJMethodDiffs().get(0).getMethod().getMethodSignature());
        assertEquals("Logging", result.getAspectJMethodDiffs().get(0).getAnnotationData().getDeleted().get(0));

        assertEquals("method2(Integer, Integer, String)", result.getAspectJMethodDiffs().get(1).getMethod().getMethodSignature());
        assertEquals("Logging2", result.getAspectJMethodDiffs().get(1).getAnnotationData().getDeleted().get(0));

        assertEquals("Logging2", result.getAspectJMethodDiffs().get(0).getAnnotationData().getModified().get(0));
        assertEquals("Logging", result.getAspectJMethodDiffs().get(1).getAnnotationData().getModified().get(0));

        assertEquals("2", result.getAspectJMethodDiffs().get(0).getAnnotationData().getId());
        assertEquals("3", result.getAspectJMethodDiffs().get(1).getAnnotationData().getId());
    }
}
