...
public class AspectJDiffManagerOsgiTest {
    ...
    public void noChanges() throws IOException {
        final String fileContent = FileUtils.readFileToString(new File("src/test/resources/ajdiff/osgi/Person_Osgi.aj"));
    ...
    public void changedBlock() throws IOException {
        final String fileContent = FileUtils.readFileToString(new File("src/test/resources/ajdiff/osgi/Person_Osgi_Changed.aj"));
        final ArgumentCaptor<JavaClass> argument = ArgumentCaptor.forClass(JavaClass.class);
        when(generatorManager.generateContentForGenerator(argument.capture(), Matchers.anyString())).thenReturn(orginalGeneratedContent());
        assertEquals("2", aspectJDiffManager.createDiff(fileContent, "").getAspectJFieldDiffs().get(0).getAnnotationData().getId());
        assertEquals(1, aspectJDiffManager.createDiff(fileContent, "").getAspectJFieldDiffs().get(0).getAnnotationData().getModified().size());
        assertEquals("Service Init", aspectJDiffManager.createDiff(fileContent, "").getAspectJFieldDiffs().get(0).getAnnotationData().getModified().get(0));    }
