...
public class AspectJDiffManagerDeletedAndChangedJavaBeanTest {
    ...
    public void changedAndDeletedBlock() throws IOException {
        final String fileContent = FileUtils.readFileToString(new File("src/test/resources/ajdiff/changedAndDeleted/javabean/Person_JavaBean_Deleted_Blocks.aj"));
        final String generatedFromJavaFile = FileUtils.readFileToString(new File("src/test/resources/ajdiff/changedAndDeleted/javabean/Person_JavaBean_Original.aj"));
        final String generatedFromJavaFile2 = FileUtils.readFileToString(new File("src/test/resources/ajdiff/changedAndDeleted/javabean/Person_JavaBean.aj"));
        ...
        assertEquals("age", result.getAspectJFieldDiffs().get(0).getField().getName());
        assertEquals("Getter", result.getAspectJFieldDiffs().get(0).getAnnotationData().getModified().get(0));

        assertEquals("name", result.getAspectJFieldDiffs().get(1).getField().getName());
        assertEquals("Setter", result.getAspectJFieldDiffs().get(1).getAnnotationData().getModified().get(0));
