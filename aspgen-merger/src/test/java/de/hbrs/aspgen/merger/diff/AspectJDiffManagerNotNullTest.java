...
public class AspectJDiffManagerNotNullTest {
    ...
    public void noChanges() throws IOException {
        final String fileContent = FileUtils.readFileToString(new File("src/test/resources/ajdiff/notnull/Person_NotNull.aj"));
    ...
    public void changedBlock() throws IOException {
        final String fileContent = FileUtils.readFileToString(new File("src/test/resources/ajdiff/notnull/Person_NotNull_Changed_Block.aj"));
        final ArgumentCaptor<JavaClass> argument = ArgumentCaptor.forClass(JavaClass.class);
        when(generatorManager.generateContentForGenerator(argument.capture(), Matchers.anyString())).thenReturn(orginalGeneratedContent());
        assertEquals("2", aspectJDiffManager.createDiff(fileContent, "").getAspectJParameterDiffs().get(0).getAnnotationData().getId());
        assertEquals(1, aspectJDiffManager.createDiff(fileContent, "").getAspectJParameterDiffs().get(0).getAnnotationData().getModified().size());
        assertEquals("NotNull", aspectJDiffManager.createDiff(fileContent, "").getAspectJParameterDiffs().get(0).getAnnotationData().getModified().get(0));    }
