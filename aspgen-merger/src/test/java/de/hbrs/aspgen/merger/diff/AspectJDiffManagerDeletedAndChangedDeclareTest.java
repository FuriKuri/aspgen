...
public class AspectJDiffManagerDeletedAndChangedDeclareTest {
    ...
    public void changedBlock() throws IOException {
        final String fileContent = FileUtils.readFileToString(new File("src/test/resources/ajdiff/changedAndDeleted/declare/Person_Declare_Deleted_Block.aj"));
        final String generatedFromJavaFile = FileUtils.readFileToString(new File("src/test/resources/ajdiff/changedAndDeleted/declare/Person_Declare_Original.aj"));
        final String generatedFromJavaFile2 = FileUtils.readFileToString(new File("src/test/resources/ajdiff/changedAndDeleted/declare/Person_Declare.aj"));
        ...
        assertEquals("setName(String)", result.getAspectJMethodDiffs().get(0).getMethod().getMethodSignature());
        assertEquals("softeningCatch2", result.getAspectJMethodDiffs().get(0).getAnnotationData().getDeleted().get(0));
        assertEquals("softeningCatch1", result.getAspectJMethodDiffs().get(0).getAnnotationData().getModified().get(0));
